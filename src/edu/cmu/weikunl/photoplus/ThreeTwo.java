...
public class ThreeTwo extends ActionBarActivity {

	ThreeTwoView v;
	private static int RESULT_LOAD_IMAGE = 1;
	
	// two boolean variables to check which collage piece the user clicked
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		v = (ThreeTwoView)findViewById(R.id.custView);
	}
	
	// the button pressed for the left piece which replaces the current image with an uploaded image
	public void btnPressedLeft(View view){
		isLeft = true;
		isRightTop = false;
		v = (ThreeTwoView)findViewById(R.id.custView);
	...
	// the button pressed for the right top piece which replaces the current image with an uploaded image
	public void btnPressedRightTop(View view){
		isLeft = false;
		isRightTop = true;
		v = (ThreeTwoView)findViewById(R.id.custView);
		((View)v).buildDrawingCache();
	...
	// the button pressed for the right bottom piece which replaces the current image with an uploaded image
	public void btnPressedRightBot(View view){
		isLeft = false;
		isRightTop = false;
		Intent i = new Intent(
				Intent.ACTION_PICK, android.provider.MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == RESULT_LOAD_IMAGE && resultCode == RESULT_OK && null != data) {
            ...
            if(isLeft){
            	v.setBitmapLeft(resizedBitmap);
            } else if (!isLeft && isRightTop){
            	v.setBitmapRightTop(resizedBitmap);
            } else {
            	v.setBitmapRightBot(resizedBitmap);
            }
        }
